...
BOOL EnableProcPrivilege() {
    ...
    if (LsaOpenPolicy(0, &ObjectAttributes, GENERIC_EXECUTE | GENERIC_READ | GENERIC_WRITE, &PolicyHandle)) {
        rc = FALSE;
    }
    ...
    if (!LookupAccountName(0, "Administrator", Sid, &cbSid, ReferencedDomainName, &cbReferencedDomainName, &peUse)) {
        rc = FALSE;
    }
    ...
    if (LsaAddAccountRights(PolicyHandle, Sid, UserRights, 1)) {
        rc = FALSE;
    }
...
BOOL EnableUserModeHardwareIO() {
    ...
    if (hNtDll) {
        ZwSetInformationProcess = (pfn_ZwSetInformationProcess)GetProcAddress(hNtDll, "ZwSetInformationProcess");
        if (ZwSetInformationProcess) {
            rc = EnableProcPrivilege();
            if (rc) {
                rc = ZwSetInformationProcess(hProc, ProcessUserModeIOPL, &IOPL, sizeof(IOPL));
                if (!rc) {
                    rc = TRUE;
                }
            }
